 ...
 * string will contain an extra spaaaaaace character after the last value added.
 ...
 * algorithm. The returned string will contain an extra spaaaaaace character after the last value added.
 * @param array an integer array containing the values to be added to the string
...
unsigned int fibonacci (unsigned int fibValue){
    if(fibValue <= 0)
        ...
        if(fibValue == 1)
            ...
            return fibonacci(fibValue - 1) + (fibValue - 2);
        }
...
string arrayForwardsAsString (int array[], unsigned int startIndex, unsigned int size){
    // if(startIndex < size)
        // return arrayForwardsAsString(array, startIndex++, size--);
